...
#'
#' @param manifest_df **TO DO: Micah, describe this.**
#' @param plateID **TO DO: Micah, describe this.**
#' @param plate_wells Numeric scalar. Number of wells on plate. Note that easyplater can currently only handle 96-well plates.
#' @inheritParams allocate_similar_samples_to_distal_wells
#'
#' @returns List of length two.
#'
#'  - The first element is data.frame with required columns "SampleID", "imbalanceFix_vec", plus columns specified in columns_for_scoring.
#'  - The second element is a tibble with columns "SampleID", "plate", "column", "row", "well", plus columns specified in columns_for_scoring.
#'
#' @export
#'
#' @examples
